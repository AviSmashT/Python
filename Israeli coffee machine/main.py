...
def coffee_prep(coffee_type):
    ...
    if (machineData.menu[coffee_type]["water"]) <= (machineData.resource["water"]) and (machineData.menu[coffee_type]["coffee"]) <= (machineData.resource["coffee"]):
        # if request is cappuccino or latte verify the amount of milk is sufficient:
        if (coffee_type == "cappuccino" or coffee_type == "latte") and (machineData.menu[coffee_type]["milk"]) <= (machineData.resource["milk"]):
            return True # resources are sufficient (water, coffee and milk)
        else:
...
def make(coffee_make):
    ...
    machineData.resource["water"] = machineData.resource["water"] - machineData.menu[coffee_make]["water"]
    machineData.resource["coffee"] = machineData.resource["coffee"] - machineData.menu[coffee_make]["coffee"]
    if coffee_make == "cappuccino" or coffee_make == "latte": # If coffee is cappuccino or latte - subtract milk
        machineData.resource["milk"] = machineData.resource["milk"] - machineData.menu[coffee_make]["milk"]
